...
#file raw/abstracts/RePORTER_PRJABS_C/FY\=*/*.csv | grep -P 'ASCII|ISO-8859' | cut -d: -f1 | xargs -P$(nproc) recode -v ISO-8859-1..UTF-8
#find raw/abstracts/RePORTER_PRJABS_C/ -type f | xargs -P$(nproc)  recode -v ISO-8859-1..UTF-8
find raw/abstracts/RePORTER_PRJABS_C/ -type f -name '*.csv' | sort | parallel --bar -k '
	FY={= ($_) = m</FY=(\d{4})/> =};
